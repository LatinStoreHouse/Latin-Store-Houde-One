...
import { initialDispatchData, DispatchData } from '@/lib/data'; // Asegúrate que DispatchData se exporte desde aquí
...
import { PlusCircle, FileDown, Search, Trash2, Copy, Edit, Calendar as CalendarIcon, PackageOpen, Warehouse, Building } from 'lucide-react';
...
// CORRECCIÓN CLAVE: Importamos useUser desde el archivo de contexto, no desde el layout.
import { useUser } from '@/context/UserProvider';
import { initialCustomerData } from '@/lib/customers';
import { DispatchForm } from '@/components/dispatch-form';
import { useToast } from '@/hooks/use-toast';

// (El resto de tu código, que ya estaba perfecto, va aquí sin cambios)
// ...
...
export default function DispatchPage() {
    ...
    const handleInputChange = (id: number, field: keyof Omit<DispatchData, 'id' | 'products'>, value: string | boolean) => {
    ...
    const handleSaveDispatch = (data: DispatchData) => {
        if (editingDispatch && editingDispatch.id !== 0) {
            ...
            const newId = dispatchData.length > 0 ? Math.max(...dispatchData.map(d => d.id)) + 1 : 1;
            setDispatchData(prev => [{...data, id: newId }, ...prev]);
            toast({ title: 'Despacho Creado', description: 'La nueva solicitud de despacho ha sido creada.'});
        }
        setIsFormOpen(false);
    }

    const filteredData = useMemo(() => dispatchData.filter(item => {
    ...
    const handleExportXLSX = () => {
        ...
        const dataToExport = filteredData.map(item => {
            const { status, factura } = getValidation_status(item.cotizacion);
            const productString = item.products.map(p => `${p.name} (x${p.quantity} - ${p.origin})`).join(', ');
            return {
